...
// Public visitors stats page (no admin login required) — same look as admin visitors
app.get('/visitors', (req, res) => {
  const rawName = db.getSetting('site_name') || 'Birdcam Live';
  const siteName = String(rawName).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/"/g, '&quot;');
  res.send(`<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Visitors – ${siteName}</title>
...
<body class="admin">
  <div class="admin-wrap">
    ...
    <main class="admin-main">
      <div class="breadcrumb"><a href="/">${siteName}</a><span class="sep">/</span><span>Visitors</span></div>
